...
class Mobilyws implements GatewayInterface
{

    const SUCCESS_CODES = [1];
    const CREDIT_SUCCESS_CODES = [1, 2];
    ...
    public function send()
    {
        $client = new \GuzzleHttp\Client([
            'base_uri' => 'http://www.mobily.ws'
        ]);
        try {
            $response = $client->post('api/msgSend.php', [
                'form_params' => [
            ...
            if (in_array($response->getBody()->getContents(), self::SUCCESS_CODES)) {
                return ['status' => true, 'response' => json_encode(json_decode($response->getBody())), 'message' => ''];
            }
            return ['status' => false, 'response' => json_encode(json_decode($response->getBody()))];
    ...
    public function getBalance()
    {
        ...
        try {
            $response = $client->post('api/balance.php', ['form_params' => [
                    'mobile' => $this->username,
                    'password' => $this->password,
                ]
            ]);
            $res = $response->getBody()->getContents();
            if (!in_array($res, self::CREDIT_SUCCESS_CODES)) {
                $b = explode('/', $res);
                return ['status' => true, 'response' => $b[1]];
            }
            return ['status' => false, 'response' => $res, 'message' => $res];
        } catch (RequestException $ex) {
            return ['status' => false, 'message' => $ex->getResponse()->getBody() !== null ? $ex->getResponse()->getBody() : $ex->getMessage()];
        } catch (ClientException $ex) {
            return ['status' => false, 'message' => $ex->getResponse()->getBody() !== null ? $ex->getResponse()->getBody() : $ex->getMessage()];
        }
    }
